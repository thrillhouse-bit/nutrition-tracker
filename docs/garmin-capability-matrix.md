...
| **Training load** | **Not ingested — UI placeholder only.** `src/components/Insights.jsx`'s "Training load" section is a deliberate, already-approved empty skeleton ("no fabricated session data... Awaiting history") — a real product decision to keep, not a bug. No code anywhere computes or stores a training-load number. | Garmin's Training Status / Training Load is a proprietary Firstbeat-derived metric — VERIFY whether it's exposed via `userMetrics`, a dedicated endpoint, or not exposed to third-party Health API partners at all (this is genuinely uncertain from public docs alone). | Unknown until partner docs are in hand — this is the biggest open question in this matrix. | Cannot propose a mapping honestly until the above is resolved. If exposed: a `training_load` metric analogous to the others above, feeding the existing Insights chart in place of its skeleton. | N/A until the data source is confirmed to exist. |
